...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      navigate(user.role === 'admin' ? '/admin' : user.role === 'authority' ? '/authority/dashboard' : '/dashboard');
    }
  ...
  useEffect(() => {
    ...
    if (token && userData) {
      try {
        ...
        window.location.href = parsed.role === 'admin' ? '/admin' : parsed.role === 'authority' ? '/authority/dashboard' : '/dashboard';
      } catch {
