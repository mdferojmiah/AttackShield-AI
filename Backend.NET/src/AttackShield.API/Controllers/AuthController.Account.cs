...
public sealed partial class AuthController
{
    ...
    public IActionResult GoogleLogin()
    {
        ...
        return Challenge(
            new AuthenticationProperties { RedirectUri = Url.Action(nameof(GoogleCallbackComplete)) },
            GoogleDefaults.AuthenticationScheme);
    }

    [HttpGet("google/callback/complete")]
    [AllowAnonymous]
    public async Task<IActionResult> GoogleCallbackComplete(CancellationToken ct)
    {
        ...
        var user = await _users.GetByGoogleIdAsync(googleId, ct) ?? await _users.GetByEmailAsync(email, ct);
        if (user is null)
        {
            user = new User
            {
                GoogleId = googleId,
                Email = email,
                Name = result.Principal.FindFirstValue(ClaimTypes.Name) ?? email,
        ...
        else if (user.GoogleId != googleId)
        {
            ...
            user.Avatar ??= result.Principal.FindFirstValue("picture");
            await _users.UpdateAsync(user.Id!, user, ct);
        }

        var token = _jwt.GenerateToken(user.Id!, user.Role);
        var publicUser = new { id = user.Id, _id = user.Id, user.Name, user.Email, role = user.Role, user.Avatar };
        var encodedToken = Uri.EscapeDataString(token);
        var encodedUser = Uri.EscapeDataString(JsonSerializer.Serialize(publicUser));
        await HttpContext.SignOutAsync("GoogleExternal");
        return Redirect($"{FrontendUrl}/login?token={encodedToken}&user={encodedUser}");
    }
    ...
    public async Task<IActionResult> Logout(CancellationToken ct)
    {
        ...
        await _ai.StopDetectionAsync(ct: ct);
