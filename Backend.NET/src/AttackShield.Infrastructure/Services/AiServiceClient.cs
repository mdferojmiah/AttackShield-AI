...
public sealed class AiServiceClient : IAiServiceClient
{
    ...
    public Task<AiCallResult> StartDetectionAsync(
        string rtspUrl,
        string? location,
        string? userId,
        string? cameraId = null,
        string? cameraName = null,
        IReadOnlyList<HitListReference>? hitList = null,
        CancellationToken ct = default)
    {
        ...
        var body = new Dictionary<string, object?>
        {
            ...
            ["hit_list"] = hitList?.Select(entry => new
            {
                id = entry.Id,
                name = entry.Name,
    ...
    public Task<AiCallResult> StopDetectionAsync(string? cameraId = null, CancellationToken ct = default)
        => PostAsync("/stop-detection", new { camera_id = cameraId }, ct);
