...
{
    Task<AiCallResult> StartDetectionAsync(
        string rtspUrl,
        string? location,
        string? userId,
    ...
    Task<AiCallResult> StopDetectionAsync(string? cameraId = null, CancellationToken ct = default);
...
public sealed record HitListReference(string Id, string Name, string ImageUrl);
