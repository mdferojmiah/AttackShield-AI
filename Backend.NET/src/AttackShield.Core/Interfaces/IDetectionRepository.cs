...
public interface IDetectionRepository : IRepository<Detection>
{
    ...
    Task<long> CountByTypeAsync(string detectionType, string? userId = null, CancellationToken ct = default);

    Task<long> CountByTypeSinceAsync(string detectionType, DateTime since, string? userId = null, CancellationToken ct = default);
