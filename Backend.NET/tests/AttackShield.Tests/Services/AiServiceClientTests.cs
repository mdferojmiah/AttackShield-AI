...
public class AiServiceClientTests
{
    ...
    [Fact]
    public async Task StartDetection_SerialisesHitListWithSnakeCaseImageUrl()
    {
        var handler = StubHandler.Returning(HttpStatusCode.OK, "{}");
        var hitList = new[] { new HitListReference("entry-1", "Jane Doe", "data:image/jpeg;base64,abc") };

        await Client(handler).StartDetectionAsync(
            "rtsp://cam/stream", "Lobby", "user1", hitList: hitList);

        using var doc = JsonDocument.Parse(handler.LastBody!);
        var entry = doc.RootElement.GetProperty("hit_list")[0];
        entry.GetProperty("id").GetString().Should().Be("entry-1");
        entry.GetProperty("name").GetString().Should().Be("Jane Doe");
        entry.GetProperty("image_url").GetString().Should().StartWith("data:image/jpeg");
        entry.TryGetProperty("imageUrl", out _).Should().BeFalse();
    }
